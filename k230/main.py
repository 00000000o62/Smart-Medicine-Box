...
MODE = "cloud"          # cloud=拍照看图, local=本地识别
...
def audio_to_text(audio_oss):
    ...
    body = {"model": "qwen-audio-turbo-latest", "input": {"messages": [
        {"role": "system", "content": [{"text": "把语音准确转成文字。对方可能说方言或口齿不清，推测正确意思。"}]},
...
def ask_qwen_text_fallback(question_text):
    ...
    body = {"model": "qwen-turbo", "messages": [
        {"role": "system", "content": "直接回答用户问题，清晰准确，不超过80字。"},
...
def async_get_voice_to_text():
    ...
    try:
        ...
        if not text:
            ...
            return
        # 用药上下文
        ctx = "你是智能药箱语音助手。用户可能是老人或视障人士，看不清楚药盒。回答要清晰、准确、大白话，不超过80字。"
        if recog_item and recog_item in MEDICINE_INFO:
            ctx += "用户面前的药是：" + recog_item + "。" + MEDICINE_INFO[recog_item]
        ctx += "回答要包含药品的主治病症、服用方法、注意事项。围绕用户问题直接回答，不超过80字。"
        print("  [3] Qwen-Turbo...")
...
def voice_serv():
    ...
    while True:
        ...
        if key.is_pressed():
            ...
            if dur > 2000:
                new = "cloud" if MODE == "local" else "local"
                print("  switch to: " + new)
                try:
        ...
        # 按键: 记录时长, 短按(<300ms)→双击检测, 长按→录音
        if key.is_pressed() and not audio_busy:
            audio_busy = True
            t0 = time.ticks_ms()
            while key.is_pressed() and time.ticks_diff(time.ticks_ms(), t0) < 300:
                time.sleep_ms(10)
            if not key.is_pressed():
                # 短按, 等500ms看是否双击
                t1 = time.ticks_ms()
                while time.ticks_diff(time.ticks_ms(), t1) < 500:
                    if key.is_pressed():
                        print("  double click -> switch to cloud")
                        with open("/sdcard/mode.txt", "w") as f: f.write("cloud")
                        rgb.show_rgb((0, 255, 0)); time.sleep_ms(300)
                        machine.reset()
                    time.sleep_ms(10)
                audio_busy = False
                continue
...
def ask_qwen_vl(image_oss):
    ...
    body = {"model": "qwen-vl-max", "input": {"messages": [{
        ...
        "content": [
            {"image": image_oss},
            {"text": "识别药品包装上的文字。必须包含：药品名称、主治什么病、一次吃几粒一天几次、主要禁忌。用户视力不好，内容准确清晰，不超过80字。"}
        ]
    }]}}
    resp = requests2.post(url, headers=headers, json_data=body, timeout=90)
    if resp.status_code == 200:
...
def cloud_mode():
    """云端模式: 按键拍照+录音 → 上传 → Qwen-VL看图+ASR → 回答"""
    print("=== Cloud Mode ===")
    key = YbKey()
    ...
    while True:
        ...
        img2.clear()
        img2.copy_from(img)
        img2.draw_string_advanced(10, 10, 16, "Hold to talk | Long 2s=switch", color=(255, 255, 0, 0))
        ...
        if key.is_pressed():
            ...
            img.save(path); i += 1
            # 判断: <300ms松手→双击检测, 长按→录音
            t_press = time.ticks_ms()
            while key.is_pressed() and time.ticks_diff(time.ticks_ms(), t_press) < 300:
                time.sleep_ms(10)
            if not key.is_pressed():
                # 短按→等500ms双击
                t1 = time.ticks_ms()
                while time.ticks_diff(time.ticks_ms(), t1) < 500:
                    if key.is_pressed():
                        print("  double click -> switch to local")
                        with open("/sdcard/mode.txt", "w") as f: f.write("local")
                        rgb.show_rgb((0, 255, 0)); time.sleep_ms(300)
                        machine.reset()
                    time.sleep_ms(10)
                try: os.remove(path)
            ...
            # 长按: 录音+上传+AI问答
            frames = []
            stream = recorder.p.open(format=paInt16, channels=1, rate=24000, input=True, frames_per_buffer=960)
            stream.volume(LEFT, 85); stream.volume(RIGHT, 85)
            while key.is_pressed():
            ...
            rgb.show_rgb((0, 0, 255))
            if frames:
                wf = wave.open("/sdcard/rec.wav", 'wb')
                wf.set_channels(1); wf.set_sampwidth(recorder.p.get_sample_size(paInt16))
                wf.set_framerate(24000); wf.write_frames(b''.join(frames)); wf.close()
            img_oss = upload_to_oss(path, "qwen-vl-max")
            try: os.remove(path)
            except: pass
            q_text = ""
            if frames:
                audio_oss = upload_to_oss("/sdcard/rec.wav", "qwen-audio-turbo")
                if audio_oss:
                    q_text = audio_to_text(audio_oss) or ""
            vl_text = ask_qwen_vl(img_oss) if img_oss else ""
            if q_text:
                ctx = "照片内容：" + (vl_text or "未知") + "。用户问：" + q_text
                reply = ask_qwen_text(ctx, "用户问药品相关问题。如果涉及具体药品，回答要包含：主治什么病、怎么服用、注意事项。不超过80字。")
            else:
                reply = vl_text
            if reply:
                print("  AI: " + reply)
                if text_to_speech_dashscope(reply):
                    _thread.start_new_thread(bg_play, ("/sdcard/tts_reply.wav",))
        # 播报时降帧率，释放DMA带宽给音频
        time.sleep_ms(200 if audio_playing else 50)
